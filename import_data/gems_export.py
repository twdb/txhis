...
cache.init_cache(CACHE_DATABASE_FILE)
...
class GEMSSite(Base):
    ...
    ODM_SQL_SiteID = Column(Integer, unique=True)
    ...
    for site in sites:
        try:
            gems_site = gems_session.query(GEMSSite).filter_by(
                ODM_SQL_SiteID=site.id).one()
        except NoResultFound:
            gems_site = GEMSSite(
                ODM_SQL_SiteID=site.id,
...
def export_timeseries(timeseries):
    for value in timeseries.values:
        data = GEMSData(
            ODM_SQL_SiteID=timeseries.site.id,
