...
class Station(Base):
    ...
    results = relationship('Event', lazy='dynamic')
...
class Event(Base):
    ...
    results = relationship('Result', lazy='dynamic', backref='event')

    # populated by backref:
    #   station: Station (many to one)
...
def create_pyhis_sites():
    file_source = cache.CacheSource(url='file:///' + TCEQ_SOURCE)
    tceq_db_session.query(Station)
