...
def create_sites_and_timeseries():
    """create all the sites and timeseries objects up front so we
    don't have to commit as often"""
    csv_files = glob.glob('/'.join([TPWD_DATA_DIR, '/request_*/*']))
                ...
                for code, cache_var in variables.items():
                    ...
                    if row[row_index]:
                        timeseries = cache.CacheTimeSeries(
                            ...
                            skip_db_lookup=True,
                            auto_commit=False)
                        cache.db_session.add(timeseries)
