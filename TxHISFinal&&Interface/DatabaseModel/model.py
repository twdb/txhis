...
from sqlalchemy import Sequence, Integer, Enum, String, Text, Boolean, DateTime, Float
...
__all__ = ['Setting', 'User', 'Sources', 'VariableMapping', 'Variables', 'Units', 'UnitConversionFormula']
...
class Sources(Base):
    ...
    SourceID = Column(Integer,Sequence('source_seq_id',optional=True),primary_key=True)
    ...
    availableParameterInfo = relationship('VariableMapping',
...
class VariableMapping(Base):
    ...
    __table_args__ = (
      UniqueConstraint('source_id', 'RemoteVariableCode', 'RemoteUnitsID',name='uix_1')
    )
    
    MappingID = Column(Integer,Sequence('VarMapping_seq_id',optional=True),primary_key=True)
    ...
    SourceInfo = relationship('Sources',
                                       primaryjoin = "VariableMapping.source_id == Sources.SourceID")
    STDcentralVariableInfo = relationship('Variables',
                                       primaryjoin = "VariableMapping.variable_id == Variables.VariableID")
      
class Variables(Base):
    __tablename__ = "Variables"
    
    VariableID = Column(Integer,Sequence('paramList_seq_id',optional=True),primary_key=True)
...
class Units(Base):
    ...
    UnitsID = Column(Integer,Sequence('Units_seq_id',optional=True),primary_key=True)
...
class UnitConversionFormula(Base):
    ...
    __table_args__ = (
        UniqueConstraint("SourceUnitsID","DestinationUnitsID", name = "MappingPair")
    )
    
    FormulaID = Column(Integer,Sequence('Formula_seq_id',optional=True),primary_key=True)
    SourceUnitsID = Column(Integer,ForeignKey('Units.UnitsID'),nullable = False)
    DestinationUnitsID = Column(Integer,ForeignKey('Units.UnitsID'),nullable = False)
    ConversionFormula = Column(String(50),nullable = False)
    
    Source_Unit = relationship('Units',
                        primaryjoin = "Units.UnitsID == UnitConversionFormula.SourceUnitsID")
    
    Destination_Unit = relationship('Units',
